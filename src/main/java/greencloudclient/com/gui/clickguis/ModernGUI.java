...
public class ModernGUI extends GuiScreen {
    ...
    private void drawSearchBar(int mouseX, int mouseY) {
        ...
        float searchX = baseWidth / 2f - (searchW / 2f);
        float searchY = baseHeight - 40;
        float searchH = 22;
        ...
        float btnX = baseWidth - 140, btnY = baseHeight - 40, btnW = 120, btnH = 22;
    ...
    protected void mouseClicked(int mouseX, int mouseY, int mouseButton) throws IOException {
        ...
        if (virtualMouseX >= baseWidth - 140 && virtualMouseX <= baseWidth - 20 && virtualMouseY >= baseHeight - 40 && virtualMouseY <= baseHeight - 18) {
            greencloudclient.com.managers.player.PositionManager.open();
            return;
        }
        
        if (virtualMouseX >= searchX && virtualMouseX <= searchX + searchW && virtualMouseY >= searchY && virtualMouseY <= searchY + 22) {
            searchFocused = true;
            return;
        } else {
            searchFocused = false;
