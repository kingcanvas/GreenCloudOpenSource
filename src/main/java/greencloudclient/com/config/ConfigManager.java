package greencloudclient.com.config;
...
public class ConfigManager {
    ...
    public void saveConfig(String configName) {
        ...
        try {
            ...
            root.add("positions", writeDraggablePositions());
    ...
    public void loadConfig(String configName) {
        ...
        try {
            ...
            if (root.has("positions")) {
                readDraggablePositions(root.getAsJsonArray("positions"));
            }
    ...
    private JsonArray writeDraggablePositions() {
        JsonArray positionsArray = new JsonArray();
        for (PositionManager.DraggableElement element : PositionManager.getInstance().elements) {
            JsonObject posObj = new JsonObject();
            posObj.addProperty("name", element.name);
            posObj.addProperty("x", element.x);
            posObj.addProperty("y", element.y);
            positionsArray.add(posObj);
        }
        return positionsArray;
    }
    
    private void readDraggablePositions(JsonArray positionsArray) {
        for (JsonElement element : positionsArray) {
            JsonObject posObj = element.getAsJsonObject();
            String name = posObj.get("name").getAsString();
            int x = posObj.get("x").getAsInt();
            int y = posObj.get("y").getAsInt();
            
            for (PositionManager.DraggableElement draggable : PositionManager.getInstance().elements) {
                if (draggable.name.equals(name)) {
                    draggable.x = x;
                    draggable.y = y;
                    break;
                }
